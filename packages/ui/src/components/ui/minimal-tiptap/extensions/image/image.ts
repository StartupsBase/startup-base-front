      ...
      setImages:
        ...
        ({ commands, editor }) => {
          ...
          if (validImages.length > 0) {
            const remoteImages = validImages.filter(
              (image): image is typeof image & { src: string } =>
                typeof image.src === "string"
            )
            const localImages = validImages.filter(
              (image): image is typeof image & { src: File } =>
            ...
            if (localImages.length > 0 && this.options.uploadFn) {
              void Promise.all(
                localImages.map(async (image) => {
                  const upload = await this.options.uploadFn!(image.src, editor)
                  const normalized =
                    typeof upload === "string"
                      ? { id: randomId(), src: upload }
                      : upload
              ...
              ).then((content) => editor.commands.insertContent(content))
            }

            return remoteImages.length > 0 ||
              (localImages.length > 0 && Boolean(this.options.uploadFn))
          }
