  ...
  return (
    <div className="fixed inset-x-2 bottom-[calc(5.5rem+env(safe-area-inset-bottom))] z-60 flex flex-col items-end sm:inset-x-auto sm:right-5 lg:bottom-5">
                ...
                <>
                  ...
                  >
                    ...
                    <textarea
                      ...
                      className="max-h-28 min-h-12 w-full resize-none bg-transparent px-2 py-1 text-sm outline-none placeholder:text-muted-foreground disabled:cursor-not-allowed"
                    ...
                    <div className="flex items-center justify-between">
                      {activeRoom?.status === "CLOSED" ? (
                        ...
                        <div className="relative flex items-center gap-1">
                          <button
                            ...
                            disabled={isBusy}
                            className="grid size-9 place-items-center rounded-full text-muted-foreground transition hover:bg-muted hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50"
                            onClick={() =>
                            ...
                            <div className="absolute bottom-11 left-0 z-20 grid w-52 grid-cols-6 gap-1 rounded-2xl border bg-popover p-2 text-popover-foreground shadow-xl">
                              {CHAT_EMOJIS.map((emoji) => (
                                <button
                                  ...
                                  type="button"
                                  aria-label={emoji}
                                  className="grid size-8 place-items-center rounded-lg text-lg transition hover:bg-muted"
                          ...
                          >
                            <HugeiconsIcon
                              ...
                              className={cn(
                                "size-5",
                                uploadImageMutation.isPending && "animate-pulse"
                              )}
                            />
                          </button>
                        </div>
                      ...
                      <button
                        ...
                        className="grid size-9 place-items-center rounded-full bg-primary text-primary-foreground transition hover:brightness-105 disabled:bg-muted disabled:text-muted-foreground"
                      >
                        <HugeiconsIcon
                          icon={ArrowUp01Icon}
                          className={cn("size-5", isBusy && "animate-pulse")}
                        />
                      </button>
                    </div>
                  </form>
      ...
      <button
        ...
        className="ml-auto grid size-14 place-items-center rounded-full bg-[#0969ff] text-white shadow-[0_16px_40px_-12px_rgba(9,105,255,.75)] transition hover:-translate-y-0.5 hover:bg-[#005be0] focus-visible:ring-2 focus-visible:ring-[#0969ff] focus-visible:ring-offset-2 focus-visible:outline-none"
        onClick={() => {
          if (isOpen) {
            setIsOpen(false)
          } else {
            ...
            {rooms.map((room) =>
              room.id == null ? null : (
                <button
                  key={room.id}
                  type="button"
                  className="flex w-full items-center gap-3 rounded-2xl p-3 text-left transition hover:bg-muted"
...
function ChatAvatar({ name }: { name?: string }) {
  ...
  return (
    <span className="grid size-10 shrink-0 place-items-center rounded-full bg-linear-to-br from-primary to-[#0969ff] text-sm font-bold text-white shadow-sm">
